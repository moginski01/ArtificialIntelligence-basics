...
class neuralNetwork:
    ...
    def fit_convol(self,expected,alpha,iterations,images_count):
        ...
        #^to tylko do tego aby uzyskać rozmiar, wiem ładniej można to zrobić
        ...
        for b in range(iterations):
            ...
            for a in range(images_count):
                ...
                # pooling_layer = np.asmatrix(np.zeros(kernel_layer.shape[0]/2,kernel_layer.shape[1]/2))
                # for c in range()


                kernel_layer_flatten = kernel_layer.reshape(-1,1)
                #tu dropout
                dropout_mask = np.asmatrix(np.random.randint(2,size=kernel_layer_flatten.shape))
        ...
        # print(correct_counter/self.input_data[0].shape[0])
        # self.convol_weights.clear()
        # for i in range(16):
        #     self.convol_weights.append(kernels[i,:].reshape(3,3).T)
        #tutaj dodać kernels do self.weight
        for i in range(16):
            self.kernels2.append(kernels[i])
    ...
    def convol_test(self,expected):
        ...
        kernels = []
        # kernels.append(kernel)
        for i in range(len(self.convol_weights)):
            # for i in range(3):
            #     kernel[0,(i*3):((i+1)*3)] = self.convol_weights[0][:,i].T
            # kernels.append(kernel)

            kernels.append(self.kernels2[i])
            # kernels.append(self.convol_weights[i])
        ...
        for a in range(10000):
            ...
            for i in range(size):
                ...
                for j in range(size):
                    ...
                    #tutaj jakis problem out of bonds
                    image_sections[section_row,:] = self.input_data[0][a,i:(i+3),j:(j+3)].T.reshape(-1,9)
                    section_row+=1

            # kernel_layer = np.dot(image_sections,kernels.T).reshape(self.convol_weights[0].shape)
            # kernel_layer_temp = image_sections * kernels.T
            kernel_layer = np.dot(image_sections,kernels.T)
            kernel_layer = self.ReLU(kernel_layer)
            kernel_layer_flatten = kernel_layer.reshape(-1,1)
            #dla flatten relu dopisać pozniej!!!
            layer_output = self.out_weights[0].T * kernel_layer_flatten
        ...
        print('procent poprawnych dane testowe: ')
...
res = myNetwork.fit_convol(expected,alpha=0.01,iterations=50,images_count=1000)
...
#& C:/Users/Mati/AppData/Local/Programs/Python/Python310/python.exe d:/Politechnika/Python/PSI/Lab5/lab5_2.py
