...
class neuralNetwork:
    ...
    def convol(self, step, padding):
        ...
        kernel = np.copy(self.weights[0])
        kernel = np.asmatrix(np.zeros((1,9)))
        for i in range(3):
            kernel[0,(i*3):((i+1)*3)] = self.weights[0][:,i].T
        ...
        kernel_layer = np.dot(image_sections,kernel.T).reshape(self.weights[0].shape)
        # kernel_layer = np.dot(image_sections,kernel.T)
...
myNetwork.load_input("input_image.txt")
